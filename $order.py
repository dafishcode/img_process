...
#paths
datapath = "/Users/dominicburrows/Documents/PhD/Imaging/181210_WT__Gc6s_SA_15%/"
...
def tiff_split(data_path, tif):   #define function tiff_split, and with two inputs
    os.mkdir(tif [0:2])           #create directory named with first two letters of file in tif
    ...
    for i in range(img.shape [0]): #i loop through number of frames in each tif file
        if counter%11 == 0:         #if counter is divisible by six, do not save
            ...
            io.imsave(arr =img [i,:,:],  fname= data_path + "prac" + tif[0:2] + "/" + "im_" +  str(names).zfill(6)  + ".tif")
